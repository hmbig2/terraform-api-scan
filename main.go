...
func buildYaml(resourceName, description string, cloudUri []CloudUri, filePath, newResourceName string) string {
    ...
    %s:
      tag: %s
      operationId: %s`, oneUrlParam.httpMethod, resourcesType, oneUrlParam.operationId)
  ...
  %s:
    %s:
      tag: %s
      operationId: %s`, resourceBase+oneUrlParam.url, oneUrlParam.httpMethod, resourcesType, oneUrlParam.operationId)
			paths = paths + yamlTemplate
		}
	}
